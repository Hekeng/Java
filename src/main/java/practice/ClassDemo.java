package src.main.java.practice;// Действие			Метод							Назначение																		Возвращаемый тип
// Чтение			get*ИмяПоля*()					Позволяет прочитать значение закрытого поля.									Тип поля (например, String)
// Запись			set*ИмяПоля*(тип значение)		Позволяет изменить значение закрытого поля (часто с дополнительной проверкой).	void
// Логический		is*ИмяПоля*()					Используется для чтения полей типа boolean.										boolean

public class ClassDemo {
	public static void main(String[] args) {
		
		Character player = new Character("Aleksei", 50);
